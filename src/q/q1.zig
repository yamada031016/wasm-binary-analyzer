// Wasmのバイナリフォーマットでは1byteのセクションIDの次に1-4bytesのセクションサイズがある。
// そのため、セクションIDを確認した後、セクションサイズを読み取り、セクションサイズのbyte数だけ読み進めることで次のセクションIDを読み取ることができる。これを続け、各セクションサイズを読み取る。
// バイナリサイズはLEB128でエンコードされているため、デコードする必要がある。
